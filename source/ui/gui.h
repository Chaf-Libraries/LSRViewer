...
namespace chaf
{
	...
	public:
		...
		public:
			 ...
			 */
			StatsView(const vkb::Stats* stats);
			 ...
			 */
			void reset_max_value(const vkb::StatIndex index);

			std::map<vkb::StatIndex, vkb::StatGraphData> graph_map;
		 ...
		 */
		Gui(vkb::RenderContext& render_context, const vkb::Window& window, const vkb::Stats* stats = nullptr,
		 ...
		 */
		void show_top_window(const std::string& app_name, const vkb::Stats* stats = nullptr, vkb::DebugInfo* debug_info = nullptr);
		 ...
		 */
		void show_debug_window(vkb::DebugInfo& debug_info, const ImVec2& position);

		/**
		 * @brief Shows a child with statistics
		 * @param stats Statistics to show
		 */
		void show_stats(const vkb::Stats& stats);
		...
		bool input_event(const vkb::InputEvent& input_event);
	...
	private:
		 ...
		 */
		void update_buffers(vkb::CommandBuffer& command_buffer, vkb::RenderFrame& render_frame);
