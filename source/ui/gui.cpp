...
namespace chaf
{
	...
	Gui::Gui(RenderContext& render_context_, const Window& window, const Stats* stats,
	...
	void Gui::show_simple_window(const std::string& name, uint32_t last_fps, std::function<void()> body)
	{
		...
		ImGui::TextUnformatted(std::string(render_context.get_device().get_gpu().get_properties().deviceName).c_str());
